...
export function Dashboard() {
  ...
  if (loading) return <Loading label="Computing metrics from synthetic data…" />;
  ...
  return (
    <>
      ...
      <div className="grid cols-4">
        <StatTile label="Total customers" value={k.total_customers} hint={`${k.buyers?.toLocaleString()} have purchased`} />
        <StatTile label="Avg order value" value={money(k.avg_order_value)} hint={`${k.completed_orders?.toLocaleString()} completed orders`} />
        <StatTile
          label="Net revenue (30d)"
          value={money(k.net_revenue_30d)}
          delta={k.revenue_mom_growth_pct}
        />
        <StatTile label="Repeat purchase rate" value={pct(k.repeat_purchase_rate_pct)} hint={`${k.repeat_buyers?.toLocaleString()} of ${k.buyers?.toLocaleString()} buyers`} />
        <StatTile label="90-day retention" value={pct(k.retention_rate_pct)} hint={`${k.retained?.toLocaleString()} of ${k.prior_window_buyers?.toLocaleString()} prior-window buyers`} />
        <StatTile label="Active customers (90d)" value={k.active_customers_90d} />
        <StatTile label="Campaign conversion" value={pct(cc.conversion_rate_pct)} hint={`${cc.conversions?.toLocaleString()} converts / ${cc.sent?.toLocaleString()} sent`} />
        <StatTile label="Revenue at risk" value={money(ar.lifetime_net_at_risk)} hint={`${ar.at_risk_customers?.toLocaleString()} lapsing high-value customers`} />
      </div>
      ...
      <div className="grid cols-2" style={{ marginTop: 16 }}>
        ...
        <ChartCard
          ...
          data={trend}
          xKey="month"
          series={[{ key: "orders", label: "Orders", color: "#1a8f5b" }]}
        />
      </div>
      ...
      <Card title="Data freshness" className="" >
        <dl className="kv">
          <dt>Latest order</dt>
